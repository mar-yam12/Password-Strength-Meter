...
def check_password_strength(password):
    ...
    if len(password) >= 8:
        score += 1;
        feedback.append("✔️Password contains at least 8 characters long");
    else:
        feedback.append("🙅🏻Password should be at least 8 characters long");

    if re.search(r'[A-Z]', password) and re.search(r"[a-z]", password):
        score += 1;
        feedback.append("✔️Password contains at least one uppercase and one lowercase letter");
    else:
        feedback.append("🙅🏻Password should contain at least one uppercase and one lowercase letter");

    if re.search(r'[0-9]', password):
        score += 1;
        feedback.append("✔️Password contains at least one number");
    else:
